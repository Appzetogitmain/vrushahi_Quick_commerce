  ...
  const fetchPolicy = async (type: PolicyType) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        let policy = response.data.find(p => p.title === type);
        
        // Fallback for legacy "Customer App Policy" if we are on Privacy Policy tab and none found
        if (!policy && type === 'Privacy Policy') {
          policy = response.data.find(p => p.title === 'Customer App Policy');
        }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white px-4 sm:px-6 py-4 border-b border-neutral-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">Customer App Policies</h1>
            <p className="text-sm text-neutral-500 mt-1">Manage Privacy Policy and Terms & Conditions for Customers</p>
          </div>
          <div className="text-sm text-neutral-600">
            <span className="text-blue-600">Home</span> / <span className="text-neutral-900">Customer Policies</span>
          </div>
        </div>
      </div>
