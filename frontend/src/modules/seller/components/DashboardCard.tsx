...
export default function DashboardCard({ icon, title, value, accentColor, onClick }: DashboardCardProps) {
  return (
    <div 
      className={`bg-white rounded-lg shadow-sm border border-neutral-200 p-3 sm:p-4 md:p-5 ${onClick ? 'cursor-pointer hover:shadow-md transition-all active:scale-[0.98]' : ''}`}
      onClick={onClick}
    >
